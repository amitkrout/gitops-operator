...
func add(mgr manager.Manager, r reconcile.Reconciler) error {
	...
	reqLogger.Info("Watching ArgoCD Route")
	...
	err = c.Watch(&source.Kind{Type: &routev1.Route{}}, &handler.EnqueueRequestForObject{}, filterPredicate(assertArgoCDRoute))
	if err != nil {
		return err
	}
...
func (r *ReconcileArgoCD) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	// update the ConsoleLink if argocd server route has changed
	if !compareRoute(found.Spec.Href, argoCDRoute.Spec.Host) {
		reqLogger.Info("Updating the existing ConsoleLink", "ConsoleLink.Name", consoleLink.Name)
		found.Spec.Href = fmt.Sprintf("https://%s", argoCDRoute.Spec.Host)
